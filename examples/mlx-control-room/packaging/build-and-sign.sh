...
# Builds, signs, and (optionally) notarizes MLX Control Room.app — adapted from the other reference
# apps' build-and-sign.sh. The one real difference: this app links TWO SDK binaries
...
#
# Runs under App Sandbox. The model download needs `com.apple.security.network.client` (in
# MLXControlRoom.entitlements) and lands in the app's container at
# ~/Library/Containers/<bundle-id>/Data/Library/Caches/huggingface/hub/ — see the README.

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
APP_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"

APP_NAME="MLX Control Room"
BIN_NAME="MLXControlRoom"
