...
# Builds, signs, and (optionally) notarizes Plate Today (Tools).app — same env-var names, same
...
# Plate Today (Tools) is the SDK's proving ground for App Sandbox compatibility: a single-process SwiftUI
...
PLATETODAYTOOLS_APP_SANDBOX="${PLATETODAYTOOLS_APP_SANDBOX:-0}"
...
ENTITLEMENTS="$BUILD_DIR/PlateTodayTools.entitlements"
...
echo "Building Plate Today (Tools) for arm64 (Location/Weather: $([ "$PLATETODAYTOOLS_INCLUDE_LOCATION_WEATHER" == "1" ] && echo included || echo excluded), Contacts: $([ "$PLATETODAYTOOLS_INCLUDE_CONTACTS" == "1" ] && echo included || echo excluded), App Sandbox: $([ "$PLATETODAYTOOLS_APP_SANDBOX" == "1" ] && echo on || echo off))..."
...
if [[ "$PLATETODAYTOOLS_APP_SANDBOX" == "1" ]]; then
  echo "Adding App Sandbox entitlement (PLATETODAYTOOLS_APP_SANDBOX=1)..."
  /usr/libexec/PlistBuddy -c "Add :com.apple.security.app-sandbox bool true" "$ENTITLEMENTS"
  # Unconditional (not gated behind PLATETODAYTOOLS_INCLUDE_LOCATION_WEATHER), since Weather and the
