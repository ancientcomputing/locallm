...
# Builds, signs, and (optionally) notarizes Security Demo.app — same structure as model-switch's
# and plate-today's packaging/build-and-sign.sh (same env-var names, same codesign/notarize
# sequence). Links TWO SDK binaries — LocalLMLabSDKCore AND LocalLMLabSDKRemote (the online-
# providers layer) — both consumed as xcframeworks, so both are copied into the bundle and
# signed. LocalLMLabSDKComponents is a source dependency and compiles into the executable.
# Calendar (EventKit) is TCC-gated: SecurityDemo.entitlements carries
# com.apple.security.personal-information.calendars and Info.plist carries
# NSCalendarsFullAccessUsageDescription. See plate-today's script for the full failure-mode
# writeups this mirrors.
...
ENTITLEMENTS="$BUILD_DIR/SecurityDemo.entitlements"
...
echo "Building Security Demo for arm64..."
...
# Calendar (EventKit) needs the personal-information.calendars entitlement — in
# SecurityDemo.entitlements. Copied and signed via --entitlements on every codesign call: that
# flag must be present on every call touching a binary or a later re-sign silently drops it.
# Keep entitlements-file comments plain ASCII — AMFIUnserializeXML rejects an em-dash in an XML
# comment with an opaque error.
cp "$SCRIPT_DIR/SecurityDemo.entitlements" "$ENTITLEMENTS"

cp "$ICON_BUILD_DIR/AppIcon.icns" "$RESOURCES_DIR/AppIcon.icns"
cp "$ICON_BUILD_DIR/Assets.car" "$RESOURCES_DIR/Assets.car"
cp "$BINARY" "$MACOS_DIR/SecurityDemo"
...
chmod +x "$MACOS_DIR/SecurityDemo"

echo "Verifying binary is arm64..."
file "$MACOS_DIR/SecurityDemo"

echo "Signing app bundle..."
for fw in "${SDK_FRAMEWORKS[@]}"; do
  codesign --force $SIGN_FLAGS --sign "$APP_IDENTITY" "$MACOS_DIR/$fw"
done
codesign --force $SIGN_FLAGS --entitlements "$ENTITLEMENTS" --sign "$APP_IDENTITY" "$MACOS_DIR/SecurityDemo"
